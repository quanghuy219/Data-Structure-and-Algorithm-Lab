...
void insertAfterCur(PHONE ph, node_ph* r){
  len++;
  node_ph* new = makeNewNode(ph);
  if (cur == tail) {
    insertAfterLast(ph);
...
void moveCur(node_ph* r, int pos){
  int i = 1;
  if (pos <= 1) {
    cur = r;
  }
...
void splitList(node_ph* r, node_ph** r1, node_ph** r2, int start, int num){
  if (start <= 1) {
    start = 1;
    cur = r;
  }
